...
message0 = """
You and the professor are in a round room with low ceilings. The walls are covered in stone tiles. 
...
Enter \'help\' if you need Monty to help with the password. 
""").lower()
    while not passed:
        if password == pw:
            passed = True
            ...
            brute_force(clue)
            print("You have to find the correct password from the possibilities Monty gives you.")
...
Enter \'hint\' if you want Monty to give you a helpful hint. 
""")
    while not passed:
        if password == pw:
            passed = True
...
To go toward the humming, type 'b'.
""").lower()
    dirt = False
    while not dirt:
        if direction == "a":
...
""").lower()
    ...
    while not dirt:
        if direction == "a":
            dirt = True
            print("""
Monty stops you from going through the door. Most passages in these old ruins have guardians. """)
            fork1(message1)
            break
        elif direction == "b":
            dirt = True
